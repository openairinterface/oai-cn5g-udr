...
- **Email subject**: `[SEC-VUL-OAI-CN5G-UDR]: "Mention the affected
  functionality here"`.
...
in the
[GitHub Issue Tracker](https://github.com/openairinterface/oai-cn5g-udr/issues),
...
- **Issue title**: `[AI-SEC-VUL-OAI-CN5G-UDR]: "Mention the affected
  functionality here"`.
- **Issue label**: apply the
  https://github.com/openairinterface/oai-cn5g-udr/labels/security
...
The [openairinterface/oai-cn5g-udr](https://github.com/openairinterface/oai-cn5g-udr)
repository contains the OAI implementation of the 5G Core **Unified Data
Repository (UDR)**, together with its configuration templates, database schema,
container images, deployment artifacts, and CI tooling. Only the 5G Core is in
scope in this repository.

Issues in the other OAI CN5G network functions (AMF, SMF, UPF, AUSF, UDM, NRF,
...
can be reported here if they are reachable through the UDR. In that case, please
state clearly which component is affected and give the commit ID of the affected
submodule in addition to the UDR commit.

Security reports are in scope when they affect the confidentiality, integrity,
or availability of the UDR running in a documented or reasonably expected
deployment.
...
- Injection into the database layer. Values taken from SBI request paths, query
  ...
  scope.
- Exposure or unauthorised modification of the subscriber credentials and
  subscription data held by the UDR, including the encrypted permanent key,
  sequence numbers, authentication status, SUPI or GPSI, and access, mobility,
  or session management subscription data, through API responses, logs,
  statistics, or error paths.
- Defects in the implemented UDR service operations (the authentication
  subscription, authentication status, access and mobility, session management,
  SDM subscription, AMF and SMF registration, and UDR configuration endpoints)
  that allow a consumer to read, modify, or delete the data of a subscriber it
  did not request, or to change the running configuration, without appropriate
  authorisation (3GPP TS 29.504).
...
- Container image, default-configuration, or provisioning issues in
  [docker/](./docker), [etc/](./etc), [openshift/](./openshift), and the
  database schema and example subscriber data in
  [build/scripts/](./build/scripts) that expose sensitive services, secrets,
  database credentials, or subscriber key material, disable important security
  controls, or grant unsafe privileges in documented or reasonably expected
  deployments.
...
- Bugs requiring local admin/root access on the UDR host with no privilege
  boundary crossed.
...
- Vulnerabilities solely caused by third-party projects or dependencies should
  ...
  the UDR deployment or requires a change in this repository, please report it
  here as well.
- Attacks that require a trusted operator role, such as an already-authorised
  UDM or peer NF deliberately misconfigured by the deployment owner, unless a
...
- Denial of service that only depends on flooding a plaintext, unprotected
  ...
  deployment, rather than on an UDR software defect.
