...
InfluenceDataStoreApi::InfluenceDataStoreApi(std::shared_ptr<Pistache::Rest::Router> rtr) { 
    router = rtr;
}
...
void InfluenceDataStoreApi::setupRoutes() {
    ...
    Routes::Get(*router, base + "/application-data/influenceData", Routes::bind(&InfluenceDataStoreApi::read_influence_data_handler, this));

    // Default handler, called when a route is not found
    router->addCustomHandler(Routes::bind(&InfluenceDataStoreApi::influence_data_store_api_default_handler, this));
}

void InfluenceDataStoreApi::read_influence_data_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    ...
    try {
      this->read_influence_data(influenceIds, dnns, snssais, internalGroupIds, supis, suppFeat, response);
...
void InfluenceDataStoreApi::influence_data_store_api_default_handler(const Pistache::Rest::Request &, Pistache::Http::ResponseWriter response) {
