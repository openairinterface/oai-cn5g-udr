...
SMFRegistrationsCollectionApiImpl::SMFRegistrationsCollectionApiImpl(std::shared_ptr<Pistache::Rest::Router> rtr, MYSQL *mysql)
    : SMFRegistrationsCollectionApi(rtr)
    {
        mysql_WitcommUDRDB = mysql;
    }

void SMFRegistrationsCollectionApiImpl::query_smf_reg_list(const std::string &ueId, const Pistache::Optional<std::string> &supportedFeatures, Pistache::Http::ResponseWriter &response) {
    MYSQL_RES *res = NULL;
    MYSQL_ROW row;
    ...
    while(row = mysql_fetch_row(res))
    {
        ...
        for (int i = 0; i<fields.size(); i++)
        {
            ...
            else if(!strcmp("emergencyServices", fields[i].c_str()) && row[i] != NULL)
            {
                if(strcmp(row[i], "0"))
            ...
            else if(!strcmp("pcscfRestorationCallbackUri", fields[i].c_str()) && row[i] != NULL)
            {
            ...
            else if(!strcmp("deregCallbackUri", fields[i].c_str()) && row[i] != NULL)
            {
                smfregistration.setDeregCallbackUri(row[i]);
            }
            else if(!strcmp("registrationReason", fields[i].c_str()) && row[i] != NULL)
            {
                RegistrationReason registrationreason;
                nlohmann::json::parse(row[i]).get_to(registrationreason);
                smfregistration.setRegistrationReason(registrationreason);
            }
            else if(!strcmp("registrationTime", fields[i].c_str()) && row[i] != NULL)
            {
